...
def get_train_val():
    ...
    def get_train(feature_name, vocab_size, len_feature):
        f = open(f'w2c_data/150/{feature_name}.txt')
        tokenizer = Tokenizer(num_words=vocab_size)
        tokenizer.fit_on_texts(f)
        f.close()

        feature_seq = []
        with open(f'w2c_data/150/{feature_name}.txt') as f:
            for text in f:
...
if not args.load_from_npy:
    ...
    def save_npy(datas, name):
        for i, data in enumerate(datas):
            np.save(
                f'tmp/{name}_{i}.npy', data)
            print(
                f'saving tmp/{name}_{i}.npy')
    ...
    DATA['X1_test'] = np.load(
        'tmp/test_0.npy', allow_pickle=True)
    DATA['X2_test'] = np.load(
        'tmp/test_1.npy', allow_pickle=True)
    DATA['X3_test'] = np.load(
        'tmp/test_2.npy', allow_pickle=True)
    DATA['X4_test'] = np.load(
        'tmp/test_3.npy', allow_pickle=True)
    DATA['X5_test'] = np.load(
        'tmp/test_4.npy', allow_pickle=True)
    DATA['X6_test'] = np.load(
        'tmp/test_5.npy', allow_pickle=True)
...
model_gender.load_weights(
    'C:/Users/yrqUni/Desktop/slc/gender_epoch_04.hdf5')


y_pred_gender = model_gender.predict(
    {
        'creative_id': DATA['X1_test'].astype('float64'),
        'ad_id': DATA['X2_test'].astype('float64'),
        'product_id': DATA['X3_test'].astype('float64'),
        'advertiser_id': DATA['X4_test'].astype('float64'),
        'industry': DATA['X5_test'].astype('float64'),
        'product_category': DATA['X6_test'].astype('float64')
    },
    batch_size=512,
)
...
ans.to_csv('tmp/submission.csv', header=True, index=False,
